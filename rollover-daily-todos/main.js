...
class RolloverSettingTab extends obsidian.PluginSettingTab {
  ...
  async display() {
    ...
    new obsidian.Setting(this.containerEl)
      .setName("Copy completed tasks from previous day")
      .setDesc(
        `If enabled, completed tasks from yesterday's daily note will be copied to today's note as plain bullet points, so you have a record of what you finished.`
      )
      .addToggle((toggle) =>
        toggle
          .setValue(this.plugin.settings.copyCompletedTasks || false)
          .onChange((value) => {
            this.plugin.settings.copyCompletedTasks = value;
            this.plugin.saveSettings();
          })
      );
    ...
    new obsidian.Setting(this.containerEl)
      ...
      .setDesc("Which heading from your template the completed tasks should go under (requires 'Copy completed tasks' to be enabled)")
      .addDropdown((dropdown) =>
        dropdown
          .addOptions({
            ...templateHeadings.reduce((acc, heading) => {
          ...
          .setValue(this.plugin?.settings.completedHeading || "none")
          .onChange((value) => {
...
class TodoParser {
  ...
  // Returns true if string s is a completed todo-item (has a done marker)
  #isCompletedTodo(s) {
    const match = s.match(/\s*[*+-] \[(.+?)\]/);
    if (!match) return false;

    const checkboxContent = match[1];
    const contentChars = this.#parseIntoChars(checkboxContent, "checkbox content");

    if (contentChars.length !== 1) return false;

    const graphemeModifiers = ['\u202E', '\u200B', '\u200C', '\u200D'];
    if (contentChars.some((char) => graphemeModifiers.includes(char))) return false;
  ...
  // Returns true if line `l` is a list item (starts with -, *, or + followed by a space)
  #isListItem(l) {
    if (l < 0 || l >= this.#lines.length) return false;
    return /^\s*[*+-] /.test(this.#lines[l]);
  }

  // Returns an array of line indices representing the parent chain for line `l`
  // (from outermost ancestor to immediate parent), only including list items
  #getParentChain(l) {
    const chain = [];
    let currentLine = l;
    let currentIndent = this.#getIndentation(currentLine);

    while (currentIndent > 0) {
      let parentIdx = currentLine - 1;
      let found = false;
      while (parentIdx >= 0) {
        const parentIndent = this.#getIndentation(parentIdx);
        if (parentIndent >= 0 && parentIndent < currentIndent && this.#isListItem(parentIdx)) {
          chain.unshift(parentIdx);
          currentLine = parentIdx;
    ...
    for (let l = 0; l < this.#lines.length; l++) {
      ...
      if (this.#isTodo(line)) {
        ...
        if (this.#withChildren && this.#hasChildren(l)) {
          ...
          // Filter out completed children — they are handled separately
          // by getCompletedTodos() when copyCompletedTasks is enabled
          const filtered = cs.filter(c => !this.#isCompletedTodo(c));
  ...
  // Returns a list of strings that represents all completed todos
  // When a completed todo is indented (a subtask), its parent chain is
  // included for context so the item isn't orphaned in the output.
  getCompletedTodos() {
    let todos = [];
    const addedLines = new Set();
    for (let l = 0; l < this.#lines.length; l++) {
      const line = this.#lines[l];
      if (this.#isCompletedTodo(line)) {
        // If this is an indented subtask, include parent chain for context
        if (this.#getIndentation(l) > 0) {
          const parentChain = this.#getParentChain(l);
          for (const pIdx of parentChain) {
            if (!addedLines.has(pIdx)) {
              todos.push(this.#lines[pIdx]);
              addedLines.add(pIdx);
            }
          }
        }
        ...
        if (this.#withChildren && this.#hasChildren(l)) {
          ...
          for (let ci = 0; ci < cs.length; ci++) {
            const childIdx = l + 1 + ci;
            if (!addedLines.has(childIdx)) {
              todos.push(cs[ci]);
              addedLines.add(childIdx);
            }
          }
          l += cs.length;
        }
...
// Utility-function to get completed todos via `TodoParser`
const getCompletedTodos = ({
  ...
  const todoParser = new TodoParser(lines, withChildren, doneStatusMarkers);
  return todoParser.getCompletedTodos();
};
...
class RolloverTodosPlugin extends obsidian.Plugin {
  async loadSettings() {
    const DEFAULT_SETTINGS = {
      ...
      copyCompletedTasks: false,
      completedHeading: "none",
    };
  ...
  async getAllCompletedTodos(file) {
    const dn = await this.app.vault.read(file);
    const dnLines = dn.split(/\r?\n|\r|\n/g);
  ...
  async rollover(file = undefined) {
    ...
    if (!this.isDailyNotesEnabled()) {
      ...
      const { templateHeading, deleteOnComplete, removeEmptyTodos, completedHeading } =
        this.settings;
      ...
      // get completed todos from yesterday, if setting enabled
      const completedTodos_yesterday = this.settings.copyCompletedTasks
        ? await this.getAllCompletedTodos(lastDailyNote)
        : [];
      ...
      if (todos_yesterday.length == 0 && completedTodos_yesterday.length == 0) {
        return;
      }
      ...
      let completedHeadingNotFoundMessage = "";
      const templateHeadingSelected = templateHeading !== "none";
      const completedHeadingSelected = completedHeading !== "none";

      if (todos_today.length > 0 || completedTodos_yesterday.length > 0) {
        ...
        // Append completed tasks from yesterday as plain bullet points
        if (completedTodos_yesterday.length > 0) {
          const completedBullets = completedTodos_yesterday.map((line) =>
            line.replace(/^(\s*)[*+-] \[.+?\] ?/, "$1- ")
          );
          const completedString = `\n${completedBullets.join("\n")}`;

          // If a completed heading is selected, try to insert under that heading
          if (completedHeadingSelected) {
            const contentAddedToHeading = dailyNoteContent.replace(
              completedHeading,
              `${completedHeading}${completedString}`
            );
            if (contentAddedToHeading == dailyNoteContent) {
              completedHeadingNotFoundMessage = `Rollover couldn't find '${completedHeading}' in today's daily note. Rolling completed tasks to end of file.`;
            } else {
              dailyNoteContent = contentAddedToHeading;
            }
          }

          // Fall back to end of file if no heading selected or heading not found
          if (!completedHeadingSelected || completedHeadingNotFoundMessage.length > 0) {
            dailyNoteContent += completedString;
          }
        }
      ...
      const completedCopiedString =
        completedTodos_yesterday.length == 0
          ? ""
          : `- ${completedTodos_yesterday.length} completed task${completedTodos_yesterday.length > 1 ? "s" : ""} copied.`;
      ...
      const emptiesToNotAddToTomorrowString =
        emptiesToNotAddToTomorrow == 0
          ...
          : "";
      const part1 =
        templateHeadingNotFoundMessage.length > 0 || completedHeadingNotFoundMessage.length > 0
          ? [templateHeadingNotFoundMessage, completedHeadingNotFoundMessage].filter(m => m.length > 0).join("\n")
          : "";
      ...
      const part4 = `${completedCopiedString}${
        completedCopiedString.length > 0 ? " " : ""
      }`;
