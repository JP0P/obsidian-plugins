...
class AutoLinkTitleSettingTab extends obsidian.PluginSettingTab {
    ...
    display() {
        ...
        new obsidian.Setting(containerEl)
            ...
            .setDesc("Personal access token for fetching Asana task and project titles. Get one at https://app.asana.com/0/my-apps")
            .addText((text) => text
...
class AutoLinkTitle extends obsidian.Plugin {
    ...
    fetchUrlTitleViaAsana(url) {
        return __awaiter(this, void 0, void 0, function* () {
            if (!this.settings.asanaToken) return "";
            const asanaUrl = new URL(url);
            if (!asanaUrl.hostname.endsWith("asana.com")) return "";
            // New URL format: /1/{workspace_gid}/project/{project_gid}/task/{task_gid}
            // Old URL format: /0/{project_gid}/{task_gid}/f
            const segments = asanaUrl.pathname.split("/").filter(Boolean);
            // Check for new-style named segments (task/project keywords)
            let resourceGid = "";
            const taskIdx = segments.indexOf("task");
            const projectIdx = segments.indexOf("project");
            if (taskIdx !== -1 && segments[taskIdx + 1] && /^\d+$/.test(segments[taskIdx + 1])) {
                resourceGid = segments[taskIdx + 1];
            } else if (projectIdx !== -1 && segments[projectIdx + 1] && /^\d+$/.test(segments[projectIdx + 1])) {
                resourceGid = segments[projectIdx + 1];
            } else {
                // Fallback: old format /0/{project_gid}/{task_gid}
                resourceGid = segments[2] && /^\d+$/.test(segments[2]) ? segments[2] : segments[1];
            }
            if (!resourceGid) return "";
            const headers = { "Authorization": `Bearer ${this.settings.asanaToken}` };
            // Try task first, fall back to project
            try {
                const taskResp = yield obsidian.requestUrl({ url: `https://app.asana.com/api/1.0/tasks/${resourceGid}?opt_fields=name`, headers });
                if (taskResp.status === 200 && taskResp.json.data && taskResp.json.data.name) {
                    return taskResp.json.data.name;
                }
            } catch (e) { /* not a task, try project */ }
            try {
                const projResp = yield obsidian.requestUrl({ url: `https://app.asana.com/api/1.0/projects/${resourceGid}?opt_fields=name`, headers });
                if (projResp.status === 200 && projResp.json.data && projResp.json.data.name) {
                    return projResp.json.data.name;
                }
            } catch (e) { /* not a project either */ }
            return "";
        });
    }
    fetchUrlTitle(url) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                let title = "";
                // Try Asana API first if token is configured and URL is from Asana
                if (this.settings.asanaToken && url.includes("asana.com")) {
                    title = yield this.fetchUrlTitleViaAsana(url);
                    console.log(`Title via Asana API: ${title}`);
